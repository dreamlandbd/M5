...
<html lang="en">
  <head>
    ...
    <h1>Role Management- Delete (<?php echo $_SESSION['username'].'/'.$_SESSION['role'] ?>) | 
    ...
    <?php if($_SESSION['username']){
        if(isAdmin()){
          $userdetail = findUser($_GET['username']);       
            ...
            echo '<input name="delete" type="text" class="form-control" 
            id="delete" value='.true.' hidden>';

            echo '<label for="email">Email</label><br/> 
              <input name="email" type="text" class="form-control" 
              id="email" value='.$userdetail[1].' disabled><br/>';            
            
            echo '<input name="role" type="text" class="form-control" 
              id="role" value='.$userdetail[3].'>';
        }
    }
    ?>
    <br/>
